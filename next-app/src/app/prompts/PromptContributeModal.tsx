...
export default function PromptContributeModal({ professions, onClose }: PromptContributeModalProps) {
  ...
  const [phase, setPhase] = useState<'sign-in' | 'form' | 'thank-you'>('sign-in')
  ...
  // If user is already signed in, skip to form phase
  useEffect(() => {
  ...
  const lockAndTransitionToForm = (id: string, firstName: string) => {
    setCheckingAuth(false)
    setUserId(id)
  ...
  const openOAuthPopup = useCallback(async (
    ...
    const interval = setInterval(async () => {
      ...
      if (user) {
        ...
        lockAndTransitionToForm(user.id, extractFirstName(user))
      }
    }, 500)
  }, [])
  ...
  const handleGoogleSuccess = useCallback(async (credential: string, nonce: string) => {
    ...
    try {
      ...
      lockAndTransitionToForm(data.user.id, extractFirstName(data.user))
    } catch (err) {
  ...
  const isSignInOrThankYou = phase === 'sign-in' || phase === 'thank-you'
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          width: isSignInOrThankYou ? 'fit-content' : '540px',
          minWidth: isSignInOrThankYou ? '575px' : undefined,
          maxWidth: '90vw',
          maxHeight: '90vh',
          padding: isSignInOrThankYou ? '0 2.75rem' : '2rem 2.75rem',
          borderRadius: '6px',
          ...(isSignInOrThankYou ? { height: '350px' } : {}),
        }}
              ...
              {checkingAuth || signingIn ? (
                ...
                <>
                  ...
                  <p
                    ...
                    style={{ fontFamily: 'var(--font-geist-sans), sans-serif' }}
                  >
                    Sign in to contribute your prompt<br />to the community toolkit
                  </p>
                </>
              )}
            </div>
