...
const CoursePage = () => {
  ...
  const [leaderFormSubmitting, setLeaderFormSubmitting] = useState(false);
  const [leaderFormSuccess, setLeaderFormSuccess] = useState(false);
              ...
              {coaches.length > 0 && (
                <div className="mt-9 mb-8">
                  ...
                  <button
                    ...
                    className="mt-4 text-[#EF0B72] hover:text-[#D10A64] font-medium transition-colors flex items-center gap-1"
                    style={{ fontSize: '15px' }}
                  >
            ...
            {leaderFormSuccess ? (
              <div className="text-center py-4">
                ...
                <p className="text-gray-600 mb-6">
                  Thank you for your interest in becoming a course leader. We'll review your application and get back to you soon.
                </p>
                <button
                  onClick={() => {
                    setShowLeaderModal(false);
                  ...
                  className="px-6 py-2 bg-[#EF0B72] hover:bg-[#D10A64] text-white font-medium rounded-lg transition-colors"
                >
                  Close
                </button>
              </div>
              ...
              <>
                ...
                <p className="text-gray-600 mb-6 text-sm">
                  Share your expertise and help shape the next generation of professionals. Fill out the form below to apply.
                </p>

                <form
                  onSubmit={async (e) => {
                    e.preventDefault();
                    setLeaderFormSubmitting(true);

                    // Create mailto link with form data
                    const subject = encodeURIComponent(`Course Leader Application - ${course?.title || 'Course'}`);
                    const body = encodeURIComponent(
                      `New Course Leader Application\n\n` +
                    ...
                    setTimeout(() => {
                      setLeaderFormSubmitting(false);
                      setLeaderFormSuccess(true);
                    }, 500);
                  }}
                  className="space-y-4"
                >
                  ...
                  <button
                    type="submit"
                    disabled={leaderFormSubmitting}
                    className="w-full py-3 bg-[#EF0B72] hover:bg-[#D10A64] disabled:bg-gray-400 text-white font-semibold rounded-lg transition-colors mt-2"
                  >
                    {leaderFormSubmitting ? 'Submitting...' : 'Submit Application'}
                  </button>
                </form>
              </>
            )}
